...
from data.data_load import load_gold

#
data_gold = load_gold()

data_gold_pct = data_gold.pct_change(fill_method=None).dropna()
...
result = test_eg(ts1=data_gold_pct['Close/Last__GOLDENCOMPANY'].values, ts2=data_gold_pct['Close/Last__GOLD'].values,
                 **kwg)

lr = LinearRegression()
lr.fit(X=data_gold_pct[['Close/Last__GOLDENCOMPANY']].values, y=data_gold_pct['Close/Last__GOLD'].values)
...
data_gold_conf = pandas.concat((below_mean_conf(data=data_gold_pct, c=0.95, p=5),
                                below_mean_conf(data=data_gold_pct, c=0.95, p=10),
                                below_mean_conf(data=data_gold_pct, c=0.95, p=20),
                                beyond_mean_conf(data=data_gold_pct, c=0.95, p=5),
                                beyond_mean_conf(data=data_gold_pct, c=0.95, p=10),
                                beyond_mean_conf(data=data_gold_pct, c=0.95, p=20)), axis=1)

data_gold_mean = pandas.concat((data_gold_pct.shift(periods=1).rolling(window=5).mean().rename(
    columns={x: '{0}__MW5'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=10).mean().rename(
                                    columns={x: '{0}__MW10'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=20).mean().rename(
                                    columns={x: '{0}__MW20'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=5).mean().rename(
                                    columns={x: '{0}__MN5'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=10).mean().rename(
                                    columns={x: '{0}__MN10'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=20).mean().rename(
                                    columns={x: '{0}__MN20'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=5).mean().rename(
                                    columns={x: '{0}__MX5'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=10).mean().rename(
                                    columns={x: '{0}__MX10'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=20).mean().rename(
                                    columns={x: '{0}__MX20'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=5).mean().rename(
                                    columns={x: '{0}__SD5'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=10).mean().rename(
                                    columns={x: '{0}__SD10'.format(x) for x in data_gold_pct.columns}),
                                data_gold_pct.shift(periods=1).rolling(window=20).mean().rename(
                                    columns={x: '{0}__SD20'.format(x) for x in data_gold_pct.columns}),
                                ), axis=1)
data_gold_lw = long_window_lag_pct(data=data_gold, n_lags=20)
data_gold_lagged_pct = lag(data_gold_pct, n_lags=20)
data_gold_conseq = conseq_check(data=data_gold_lagged_pct, max_lags=[5, 10, 20])

data_joint = data_gold_lagged_pct.copy()
# data_joint = data_joint.merge(right=data_gold_lw, left_index=True, right_index=True, how='inner')
# data_joint = data_joint.merge(right=data_gold_mean, left_index=True, right_index=True, how='inner')
# data_joint = data_joint.merge(right=data_gold_conseq, left_index=True, right_index=True, how='inner')
# data_joint = data_joint.merge(right=data_gold_conf, left_index=True, right_index=True, how='inner')
...
# Y = ((data_joint['Close/Last__GOLDENCOMPANY'] - data_joint['Close/Last__GOLD']) > 0).astype(dtype=int).values

"""
ax = (data_joint['Close/Last__GOLD'] - lr.coef_[0] * data_joint['Close/Last__GOLDENCOMPANY']).hist(bins=100)
fig = ax.get_figure()
fig.savefig('C:/Users/azizove/Desktop/fig1.png')


adfuller(x=(data_joint['Close/Last__GOLD'] - lr.coef_[0] * data_joint['Close/Last__GOLDENCOMPANY']), maxlag=None)
(data_joint['Close/Last__GOLD'] - lr.coef_[0] * data_joint['Close/Last__GOLDENCOMPANY']).describe(percentiles=(0.0, 0.01, 0.05, 0.1, 0.4, 0.45, 0.49, 0.5, 0.51, 0.55, 0.6, 0.9, 0.95, 0.99, 1.0))
"""
# """
spread = (data_joint['Close/Last__GOLD'] - lr.coef_[0] * data_joint['Close/Last__GOLDENCOMPANY']).values
...
Y = ((data_joint['Close/Last__GOLD'] - lr.coef_[0] * data_joint['Close/Last__GOLDENCOMPANY']).apply(func=sec)).astype(dtype=int).values
# """
# Y = ((data_joint['Close/Last__GOLD'] - lr.coef_[0] * data_joint['Close/Last__GOLDENCOMPANY']) > 0).astype(dtype=int).values
