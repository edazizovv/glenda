...
from data.data_load import load_coal_monthly

#
data_coal = load_coal_monthly()
...
resulted_coal = search_hb(data=data_coal_pct.values, test=test_eg, test_kwargs=kwg, alpha=alpha)
resulted_platinum = search_hb(data=data_platinum_pct.values, test=test_eg, test_kwargs=kwg, alpha=alpha)


# resulted_gas = search_hb(data=data_gas_pct[['Close/Last__GASNCOMPANY', 'Close/Last__GAS']].values, 
# test=test_eg, test_kwargs=kwg, alpha=alpha)
# resulted_gas = {x: (data_gas_pct.columns.values[resulted_gas[x][0]],
#                        data_gas_pct.columns.values[resulted_gas[x][1]]) for x in resulted_gas.keys()}
"""
