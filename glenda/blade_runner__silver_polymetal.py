...
from data.data_load import load_silver_polymetal

#
data_silver = load_silver_polymetal()

data_silver = data_silver.drop(columns=['Volume__POLYMETAL'])
...
result = test_eg(ts1=data_silver_pct['Close__POLYMETAL'].values, ts2=data_silver_pct['Close/Last__SILVER'].values,
                 **kwg)

lr = LinearRegression()
lr.fit(X=data_silver_pct[['Close__POLYMETAL']].values, y=data_silver_pct['Close/Last__SILVER'].values)
...
# Y = ((data_joint['Close__POLYMETAL'] - data_joint['Close/Last__SILVER']) > 0).astype(dtype=int).values

"""
ax = (data_joint['Close/Last__SILVER'] - lr.coef_[0] * data_joint['Close__POLYMETAL']).hist(bins=100)
fig = ax.get_figure()
fig.savefig('C:/Users/azizove/Desktop/fig1.png')


adfuller(x=(data_joint['Close/Last__SILVER'] - lr.coef_[0] * data_joint['Close__POLYMETAL']), maxlag=None)
(data_joint['Close/Last__SILVER'] - lr.coef_[0] * data_joint['Close__POLYMETAL']).describe(percentiles=(0.0, 0.01, 0.05, 0.1, 0.4, 0.45, 0.49, 0.5, 0.51, 0.55, 0.6, 0.9, 0.95, 0.99, 1.0))
"""
"""
spread = (data_joint['Close/Last__SILVER'] - lr.coef_[0] * data_joint['Close__POLYMETAL']).values
...
Y = ((data_joint['Close/Last__SILVER'] - lr.coef_[0] * data_joint['Close__POLYMETAL']).apply(func=sec)).astype(dtype=int).values
"""
Y = ((data_joint['Close/Last__SILVER'] - lr.coef_[0] * data_joint['Close__POLYMETAL']) > 0).astype(dtype=int).values
