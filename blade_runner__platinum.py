...
from data.data_load import load_platinum

#
data_platinum = load_platinum()

data_platinum_pct = data_platinum.pct_change(fill_method=None).dropna()
...
resulted_platinum = search_hb(data=data_platinum_pct.values, test=test_eg, test_kwargs=kwg, alpha=alpha)

lr = LinearRegression()
lr.fit(X=data_platinum_pct[['Close__ANGPY']].values, y=data_platinum_pct[' value__PLATINUM'].values)
...
data_platinum_conf = pandas.concat((below_mean_conf(data=data_platinum_pct, c=0.95, p=5),
                                    below_mean_conf(data=data_platinum_pct, c=0.95, p=10),
                                    below_mean_conf(data=data_platinum_pct, c=0.95, p=20),
                                    beyond_mean_conf(data=data_platinum_pct, c=0.95, p=5),
                                    beyond_mean_conf(data=data_platinum_pct, c=0.95, p=10),
                                    beyond_mean_conf(data=data_platinum_pct, c=0.95, p=20)), axis=1)

data_platinum_mean = pandas.concat((data_platinum_pct.shift(periods=1).rolling(window=5).mean().rename(
    columns={x: '{0}__MW5'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=10).mean().rename(
                                    columns={x: '{0}__MW10'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=20).mean().rename(
                                    columns={x: '{0}__MW20'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=5).mean().rename(
                                    columns={x: '{0}__MN5'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=10).mean().rename(
                                    columns={x: '{0}__MN10'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=20).mean().rename(
                                    columns={x: '{0}__MN20'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=5).mean().rename(
                                    columns={x: '{0}__MX5'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=10).mean().rename(
                                    columns={x: '{0}__MX10'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=20).mean().rename(
                                    columns={x: '{0}__MX20'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=5).mean().rename(
                                    columns={x: '{0}__SD5'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=10).mean().rename(
                                    columns={x: '{0}__SD10'.format(x) for x in data_platinum_pct.columns}),
                                data_platinum_pct.shift(periods=1).rolling(window=20).mean().rename(
                                    columns={x: '{0}__SD20'.format(x) for x in data_platinum_pct.columns}),
                                ), axis=1)
data_platinum_lw = long_window_lag_pct(data=data_platinum, n_lags=20)
data_platinum_lagged_pct = lag(data_platinum_pct, n_lags=20)
data_platinum_conseq = conseq_check(data=data_platinum_lagged_pct, max_lags=[5, 10, 20])

data_joint = data_platinum_lagged_pct.copy()
data_joint = data_joint.merge(right=data_platinum_lw, left_index=True, right_index=True, how='inner')
data_joint = data_joint.merge(right=data_platinum_mean, left_index=True, right_index=True, how='inner')
# data_joint = data_joint.merge(right=data_platinum_conseq, left_index=True, right_index=True, how='inner')
# data_joint = data_joint.merge(right=data_platinum_conf, left_index=True, right_index=True, how='inner')
...
# Y = ((data_joint['Close__ANGPY'] - data_joint[' value__PLATINUM']) > 0).astype(dtype=int).values

"""
ax = (data_joint[' value__PLATINUM'] - lr.coef_[0] * data_joint['Close__ANGPY']).hist(bins=100)
fig = ax.get_figure()
fig.savefig('C:/Users/azizove/Desktop/fig1.png')


adfuller(x=(data_joint[' value__PLATINUM'] - lr.coef_[0] * data_joint['Close__ANGPY']), maxlag=None)
(data_joint[' value__PLATINUM'] - lr.coef_[0] * data_joint['Close__ANGPY']).describe(percentiles=(0.0, 0.01, 0.05, 0.1, 0.4, 0.45, 0.49, 0.5, 0.51, 0.55, 0.6, 0.9, 0.95, 0.99, 1.0))
"""
# """
spread = (data_joint[' value__PLATINUM'] - lr.coef_[0] * data_joint['Close__ANGPY']).values
...
Y = ((data_joint[' value__PLATINUM'] - lr.coef_[0] * data_joint['Close__ANGPY']).apply(func=sec)).astype(dtype=int).values
# """
# Y = ((data_joint[' value__PLATINUM'] - lr.coef_[0] * data_joint['Close__ANGPY']) > 0).astype(dtype=int).values
